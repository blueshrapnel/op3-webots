...
from launch.actions import EmitEvent, OpaqueFunction, RegisterEventHandler, TimerAction
...
def spawn_controller(context, webots_action, gain_file, robot_name):
    return [
        Node(
            ...
            additional_env={
                "WEBOTS_CONTROLLER_URL": f"tcp://localhost:1234/{robot_name}"
            },
        )
    ]
            ...
            parameters=[
                {
                    ...
                    "offset_file_path": os.path.join(
                        pkg_share, "config", "offset.yaml"
                    ),
                    "robot_file_path": os.path.join(pkg_share, "config", "OP3.robot"),
                    "init_file_path": os.path.join(
                        pkg_share, "config", "dxl_init_OP3.yaml"
                    ),
                    "team_number": 0,
                    "player_number": 1,
                }
